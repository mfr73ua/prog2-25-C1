...
# 2. Login
print("\n🔐 2. Iniciando sesión...")
login_data = {"username": "adrian_ruiz", "password": "adrian123"}
...
ruta_manual_data = {
    "username": "adrian_ruiz",
    "password": "adrian123",
    "origen": "Playa de San Juan",
    "intermedios": [str(i) for i in ['Centro Comercial Torre Golf', 'Av. de Niza']],
    "destino": "Cabo de las Huertas",
    "modo": "bike",
    "nombre": "ruta_bici_adrian"
}
...
print("\n📦 6. Obteniendo rutas del usuario 'adrian_ruiz'...")
resp = requests.get(f"{BASE_URL}/usuarios/adrian_ruiz/rutas")
...
# 9. Descargar PDF
print("\n📄 9. Descargando PDF de la ruta 'ruta_bici_adrian'...")
resp = requests.get(f"{BASE_URL}/rutas/ruta_bici_adrian/pdf")
if resp.status_code == 200:
    with open("test6.pdf", "wb") as f:
        f.write(resp.content)
    print("✅ PDF descargado como 'test6.pdf'")
else:
    print("⚠️ No se encontró el PDF:", resp.status_code)

# 10. Descargar HTML
print("\n🌐 10. Descargando HTML de la ruta 'ruta_bici_adrian'...")
resp = requests.get(f"{BASE_URL}/rutas/ruta_bici_adrian/html")
if resp.status_code == 200:
    with open("test6.html", "wb") as f:
        f.write(resp.content)
    print("✅ HTML descargado como 'test6.html'")
else:
